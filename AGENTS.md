...
- Task-Specific Thinking: Extraction and formatting use minimal thinking. Drafting, solving, adjudication, semantic grading, and student explanations use high thinking. Do not add route-local thinking settings unless the shared profile is intentionally updated.
...
- LaTeX Scope: Use `$...$` only for actual inline mathematical expressions and `$$...$$` only for standalone equations, aligned work, or tables. Ordinary prose numbers, dates, labels, option letters, and names do not require math delimiters.
