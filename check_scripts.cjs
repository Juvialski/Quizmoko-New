...
while ((match = scriptRegex.exec(content)) !== null) {
  ...
  if (match[0].includes('type="module"')) {
     try { acorn.parse(code, {ecmaVersion: 2020, sourceType: 'module'}); }
     catch(e) { console.error(`Error in module script ${count}:`, e.message, code.substring(e.loc.line - 10, e.loc.line + 10)); }
     continue;
  }
