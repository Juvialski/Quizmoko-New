...
8. DIAGRAM REQUIREMENT: exactly {images_count} question texts in this batch must contain one [TIKZ]...[/TIKZ] block when {images_count} is greater than 0. The question plan marks each item as diagram_required="yes" or diagram_required="no". Every item marked diagram_required="yes" MUST contain exactly one useful TikZ diagram. Every item marked diagram_required="no" MUST contain no [TIKZ] block. For graph-friendly topics such as derivatives, geometry, coordinate-plane mathematics, statistics, or physics graphs, make the diagram mathematically meaningful (for example a function graph, tangent/secant picture, coordinate graph, or labeled geometric figure), not decorative. Use BASE TIKZ ONLY: do not use pgfplots, \begin{axis}, \addplot, \documentclass, or \usepackage. A coordinate graph may use \draw with axes/grid and a base-TikZ plot. The diagram must not reveal the answer.
