...
const uiTest = 'test/ui-consistency.test.ts';
let uiContent = fs.readFileSync(uiTest, 'utf8');
const uiMarker = "test('worksheet source identity UI regressions'";
if (!uiContent.includes(uiMarker)) {
  uiContent += `\n\ntest('worksheet source identity UI regressions', () => {\n  for (const name of ['worksheet_upload.ejs', 'worksheet_answers_upload.ejs']) {\n    const content = fs.readFileSync(path.join(VIEWS_DIR, name), 'utf8');\n    assert.ok(\n      content.includes('return \\`roman:\\${romanMatch[1].toUpperCase()}:\\${romanMatch[2].trim().toLowerCase()}\\`;'),\n      \\`\\${name} must keep Roman source identifiers distinct from Arabic identifiers during recovery matching\\`\n    );\n    assert.ok(\n      !content.includes('return roman !== null\\n                ? \\`numeric:\\${roman}:'),\n      \\`\\${name} must not normalize Roman source identifiers into Arabic match keys\\`\n    );\n  }\n\n  const printable = fs.readFileSync(path.join(VIEWS_DIR, 'worksheet.ejs'), 'utf8');\n  assert.ok(printable.includes('q.source_id !== undefined'), 'printable worksheet must fall back to source_id');\n});\n`;
  fs.writeFileSync(uiTest, uiContent);
}
