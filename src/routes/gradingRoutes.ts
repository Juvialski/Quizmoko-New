...
router.post(
  ...
  async (req, res) => {
    ...
    return withAttemptLock(quizId, sessionId, async () => {
      ...
      if (io) io.to(`quiz_${quizId}`).emit('progressive_result_update', { result_id: resultId });
...
CRUCIAL: You MUST enclose ALL mathematical expressions, numbers, equations, counts, measurements, percentages, and standalone numbers (except for Identification answers) inside your feedback with LaTeX dollar signs (e.g., $x^2$, $130/10$, $\$$40, $15\%$, $-42$, $10$ meters). Do NOT use asterisks for math.
Return only {"explanation":"..."}.`;
        ...
        if (parsed && typeof parsed.explanation === 'string' && parsed.explanation.trim()) {
          return res.json({ success: true, explanation: parsed.explanation.slice(0, MAX_FEEDBACK_LENGTH) });
        }
      } catch (error) {
