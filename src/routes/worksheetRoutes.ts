...
const SINGLE_CHECKER_SCHEMA = {
  ...
  properties: {
    ...
    corrected_type: { type: Type.STRING, enum: [...ALLOWED_QUESTION_TYPE_VALUES] },
...
function parseIndependentSolverOutput(text: string, expectedSourceId: string): Record<string, unknown> {
  const parsed = safeParseJSON(text || '');
  if (!Array.isArray(parsed) || parsed.length !== 1 || !parsed[0] || typeof parsed[0] !== 'object') {
    throw new Error('Solver did not return exactly one result');
  }
  const output = parsed[0] as Record<string, unknown>;
  if (Number(output.source_index) !== 0) throw new Error('Solver returned the wrong source_index');
  if (normalizeWorksheetSourceId(output.source_id) !== expectedSourceId) {
    throw new Error('Solver returned the wrong source_id');
  }
  return output;
}
...
async function requestWorksheetAdjudication(input: {
  ai: NonNullable<ReturnType<typeof getGeminiClient>>;
...
${MANDATORY_MATH_FEEDBACK_RULE}`;
  ...
  try {
    ...
    if (!parsed || Array.isArray(parsed) || typeof parsed !== 'object') {
      return { status: 'failed', reason: 'Adjudicator returned invalid JSON.' };
    }
    return {
      status: 'fulfilled',
...
async function solveWorksheetQuestionWithConsensus(input: {
  ai: NonNullable<ReturnType<typeof getGeminiClient>>;
  rawQuestion: Record<string, unknown>;
  subject: string;
  topic: string;
  requestedModel: string;
}): Promise<ReturnType<typeof adjudicateWorksheetSolverCandidates>> {
  const sourceId = getWorksheetSourceId(input.rawQuestion);
  if (!sourceId) {
    return adjudicateWorksheetSolverCandidates(input.rawQuestion, [
      { model: 'source-validation', status: 'failed', error: 'Missing stable source ID.' },
      { model: 'source-validation-2', status: 'failed', error: 'Missing stable source ID.' }
    ]);
  }
  ...
  const runModel = async (model: string): Promise<Record<string, unknown>> => {
    const contents: any[] = [];
    const originalText = input.rawQuestion.raw_text || input.rawQuestion.question || input.rawQuestion.statement || '';
    const prepared = prepareVisionText(originalText, contents);
    const promptQuestion = {
      ...stripWorksheetSolverState(input.rawQuestion),
      question: prepared.text,
    ...
    return parseIndependentSolverOutput(response.text || '', sourceId);
  };

  const settled = await Promise.allSettled([runModel(primaryModel), runModel(secondaryModel)]);
  const candidates: IndependentSolverCandidate[] = settled.map((result, index) => ({
    model: index === 0 ? primaryModel : secondaryModel,
    status: result.status === 'fulfilled' ? 'fulfilled' : 'failed',
    ...(result.status === 'fulfilled' ? { output: result.value } : { error: String(result.reason?.message || result.reason || 'Solver failed') })
  }));
  let consensus = adjudicateWorksheetSolverCandidates(input.rawQuestion, candidates);
  if (consensus.diagnostics.some(item => item.code === 'solver_disagreement')) {
    ...
    consensus = adjudicateWorksheetSolverCandidates(input.rawQuestion, candidates, adjudicator);
  }

  const rawVerification = input.rawQuestion.verification;
  ...
  const goldenAnswer = input.rawQuestion.answer;
  const agreesWithGolden = consensus.publishable
    && areCanonicalWorksheetAnswersEquivalent(input.rawQuestion, goldenAnswer, consensus.question.answer);
  ...
  if (!agreesWithGolden) {
    ...
    const verification = {
      ...
      solver_models: [primaryModel, secondaryModel]
    };
    return {
      publishable: false,
      question: { ...input.rawQuestion, answer: goldenAnswer, verification, worksheet_qa: qa },
      verification,
      worksheet_qa: qa,
      diagnostics
    };
    ...
    try {
      ...
      for (let i = 0; i < totalQuestions; i += batchSize) {
        ...
        const batchPromises = batchQuestions.map(async (rawQ: any, index: number) => {
          const qIdx = i + index;
          const result = await solveWorksheetQuestionWithConsensus({
            ai,
            rawQuestion: rawQ,
          ...
          if (!result.publishable) {
            throw new WorksheetReviewRequiredError(
              `Question ${getWorksheetSourceId(rawQ) || qIdx + 1} requires teacher review.`,
              result.diagnostics
            );
          }
          setWorksheetProgress(session_id, {
            message: `✅ Q${qIdx + 1}/${totalQuestions}: independent solvers verified the answer.`,
            percentage: Math.round(15 + ((qIdx + 1) / totalQuestions) * 75),
            status: 'processing'
          });
  ...
  try {
    ...
    if (ai) {
        ...
        for (let i = 0; i < totalQs; i += batchSize) {
          ...
          const batchQuestions = sourceQuestions.slice(i, batchEnd);
          ...
          const batchPromises = batchQuestions.map(async (rawQ: any, index: number) => {
            ...
            if (!result.publishable && !isQuestionExplicitlyApproved(rawQ)) {
              throw new WorksheetReviewRequiredError(
                `Question ${getWorksheetSourceId(rawQ) || qIdx + 1} requires teacher review.`,
                result.diagnostics
              );
            ...
            setWorksheetProgress(session_id, {
              message: result.publishable
                ? `✅ Q${qIdx + 1}/${totalQs}: independent solvers verified the answer.`
                : `✅ Q${qIdx + 1}/${totalQs}: teacher-approved review result retained.`,
              percentage: Math.round(20 + ((qIdx + 1) / totalQs) * 75),
              status: 'processing'
            });
            return result.question;
          });
